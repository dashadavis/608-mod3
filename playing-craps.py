...
trials = 6_000_000 
# 6 million did not work the first time, so did it with 6,000, fixed a couple other errors, then 6 million worked

#Simulate rolls
for roll in range(trials):
  face = random.randrange(1, 7) + random.randrange(1, 7)

#increment appropriate counter # indention was wrong, if should be lined up with face
  if face == 1:
    frequency1 += 1
  elif face == 2:
    frequency2 += 1
